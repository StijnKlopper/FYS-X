...
public class LSystemsGenerator : MonoBehaviour
{
    ...
    public void Generate()
    {
        ...
        Tree = Instantiate(treeParent, position, Quaternion.identity);
        ...
        for (int i = 0; i < currentString.Length; i++)
        {
            switch (currentString[i])
            {
                case 'F':
                    ...
                    GameObject fLine = currentString[(i + 1) % currentString.Length] == 'X' || currentString[(i + 3) % currentString.Length] == 'F' && currentString[(i + 4) % currentString.Length] == 'X' ? Instantiate(leaf, position, Quaternion.identity) : Instantiate(branch, position, Quaternion.identity);
...
/*                    fLine.GetComponent<LineRenderer>().SetPosition(0, initialPosition);
                    ...
                    //fLine.GetComponent<MeshFilter>().transform.position = initialPosition;
